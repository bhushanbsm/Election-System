...
//do voting process here
		public function voting($id)
		{
			...
			$this->form_validation->set_rules('aadharno', 'Aadhar', 'trim|required|min_length[12]|max_length[12]|callback_check_aadhar');
			if ($this->form_validation->run() == FALSE) {
				$data['result']="";
			} else {	//if result is true add vote and aadhar no to db
				$vote_result=$this->aadhar->vote($id);
...
//check for aadhar no in db if aadhar no is present send True otherwise False
		...
		{
			if ($this->aadhar->get_aadhar()==TRUE) {
